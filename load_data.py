...
def setup_database():
    """
    Creates the new, robust database schema and populates definition tables.
    """
    print("--- Setting up new database schema and definitions ---")
    
    drop_queries = [
        "DROP TABLE IF EXISTS Results;", "DROP TABLE IF EXISTS Meets;",
        "DROP TABLE IF EXISTS Athletes;", "DROP TABLE IF EXISTS Apparatus;",
        "DROP TABLE IF EXISTS Events;", "DROP TABLE IF EXISTS Disciplines;"
    ]
    schema_queries = [
        "CREATE TABLE Disciplines (discipline_id INTEGER PRIMARY KEY, discipline_name TEXT NOT NULL UNIQUE);",
        "CREATE TABLE Apparatus (apparatus_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, discipline_id INTEGER NOT NULL, sort_order INTEGER, FOREIGN KEY (discipline_id) REFERENCES Disciplines (discipline_id), UNIQUE(name, discipline_id));",
        "CREATE TABLE Athletes (athlete_id INTEGER PRIMARY KEY AUTOINCREMENT, full_name TEXT NOT NULL, club TEXT, gender TEXT, UNIQUE(full_name, club));",
        "CREATE TABLE Meets (meet_db_id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, source_meet_id TEXT NOT NULL, name TEXT, start_date_iso TEXT, location TEXT, year INTEGER, UNIQUE(source, source_meet_id));",
        "CREATE TABLE Results (result_id INTEGER PRIMARY KEY AUTOINCREMENT, meet_db_id INTEGER NOT NULL, athlete_id INTEGER NOT NULL, apparatus_id INTEGER NOT NULL, score_d REAL, score_final REAL, score_text TEXT, rank_numeric INTEGER, rank_text TEXT, details_json TEXT, FOREIGN KEY (meet_db_id) REFERENCES Meets (meet_db_id), FOREIGN KEY (athlete_id) REFERENCES Athletes (athlete_id), FOREIGN KEY (apparatus_id) REFERENCES Apparatus (apparatus_id));"
    ]
    try:
...
def load_meet_manifest(manifest_file):
    ...
    """
    print(f"--- Loading meet manifest from '{manifest_file}' ---")
    try:
        ...
        print(f"Successfully loaded details for {len(manifest)} meets into memory.")
        return manifest
    except (FileNotFoundError, KeyError) as e:
        print(f"Warning: Could not load meet manifest file. Meet details will be incomplete. Error: {e}")
        return {} 
...
def get_or_create_meet(conn, source, source_meet_id, meet_details, cache):
    ...
    if result:
        ...
        print(f"  -> New meet added to DB: '{meet_details.get('name')}' (ID: {meet_db_id})")
...
# --- Helper functions (Restored to full, readable format) ---
